...
| Surface | Treatment |
|---|---|
| Exam study guide (`pages/wiki/WikiExam.tsx`) | `components/wiki/KeystoneStrip.tsx` — the gold panel that names all of the exam's keystones with per-concept mastery dots and a mastered-count bar, and opens any of them in the concept popup. It sits **below the learning objectives** (placed by the `KEYSTONE_MARKER` that `markKeystoneStrip` inserts at the end of that section — see `components/wiki/WikiArticle.tsx`) and is **collapsed by default**: the header is one line, `Keystone concepts n/total` plus a chevron, and the reader's choice is remembered in `localStorage` |
