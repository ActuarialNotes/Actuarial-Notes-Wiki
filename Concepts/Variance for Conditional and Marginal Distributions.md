The **Variance for Conditional and Marginal Distributions** measures spread at two different levels: the marginal variance describes the spread of $X$ across all values of $Y$, while the conditional variance $\text{Var}(X \mid Y=y)$ measures the spread of $X$ within the subpopulation where $Y=y$.
