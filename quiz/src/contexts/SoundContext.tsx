...
// Paths that returned a load error this session — skip retrying them
...
function synthStart(ctx: AudioContext, vol: number) {
  ...
  gain.gain.setValueAtTime(0, ctx.currentTime)
  gain.gain.linearRampToValueAtTime(vol * 0.7, ctx.currentTime + 0.02)
...
function synthFallback(event: SoundEvent) {
  try {
