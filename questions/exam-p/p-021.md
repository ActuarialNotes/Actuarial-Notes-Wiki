---
id: "p-021"
exam: "Probability"
topic: "Bayes Theorem"
learning_objective: "General Probability"
difficulty: medium
type: multiple-choice
wiki_link:
  - Concepts/Bayes+Theorem
  - Concepts/Conditional+Probability
answer: "D"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:104aa1debbc1600333961cf7ee46463a844bf6e14e488428ee278cab84cd6cbd
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-021.md
---

A health study tracked a group of persons for five years. At the beginning of the study, 20% were classified as heavy smokers, 30% as light smokers, and 50% as nonsmokers.

Results of the study showed that light smokers were twice as likely as nonsmokers to die during the five-year study, but only half as likely as heavy smokers.

A randomly selected participant from the study died during the five-year period.

Calculate the probability that the participant was a heavy smoker.

- A) 0.20
- B) 0.25
- C) 0.35
- D) 0.42
- E) 0.57

## Explanation

Let $H$ = heavy smoker, $L$ = light smoker, $N$ = nonsmoker, $D$ = death within five years.

Given: $P[D|L] = 2P[D|N]$ and $P[D|L] = \frac{1}{2}P[D|H]$.

$$P[H|D] = \frac{P[D|H]P[H]}{P[D|N]P[N] + P[D|L]P[L] + P[D|H]P[H]}$$

Let $P[D|L] = p$. Then $P[D|N] = p/2$, $P[D|H] = 2p$.

$$
\begin{align*}
= \frac{2p(0.2)}{\frac{p}{2}(0.5) + p(0.3) + 2p(0.2)} &= \frac{0.4}{0.25 + 0.30 + 0.40} \\
                                                      &= \frac{0.4}{0.95} \\
                                                      &\approx 0.42
\end{align*}
$$
