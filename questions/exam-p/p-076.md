---
id: "p-076"
exam: "Probability"
topic: "Variance and Standard Deviation"
learning_objective: "Univariate Random Variables"
difficulty: easy
type: multiple-choice
wiki_link:
  - Concepts/Variance+and+Standard+Deviation
  - Concepts/Discrete+Univariate+Distributions
  - Concepts/Variance+for+Conditional+and+Marginal+Distributions
answer: "B"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:3f99b8dfda2f3faea629af6c05edd6cb967117e74252df1bd447d95b745e6315
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-076.md
---

A car dealership sells 0, 1, or 2 luxury cars on any day. When selling a car, the dealer also tries to persuade the customer to buy an extended warranty for the car. Let $X$ denote the number of luxury cars sold in a given day, and let $Y$ denote the number of extended warranties sold.

$$P[X=0, Y=0] = 1/6$$
$$P[X=1, Y=0] = 1/12$$
$$P[X=1, Y=1] = 1/6$$
$$P[X=2, Y=0] = 1/12$$
$$P[X=2, Y=1] = 1/3$$
$$P[X=2, Y=2] = 1/6$$

Calculate the variance of $X$.

- A) 0.47
- B) 0.58
- C) 0.83
- D) 1.42
- E) 2.58

## Explanation

First, find the marginal probability distribution of $X$ by summing the probabilities over all values of $Y$:
* $P[X=0] = P[X=0, Y=0] = 1/6$
* $P[X=1] = P[X=1, Y=0] + P[X=1, Y=1] = 1/12 + 1/6 = 3/12 = 1/4$
* $P[X=2] = P[X=2, Y=0] + P[X=2, Y=1] + P[X=2, Y=2] = 1/12 + 1/3 + 1/6 = 1/12 + 4/12 + 2/12 = 7/12$

Calculate the first moment $E[X]$:
$$
\begin{align*}
E[X] &= 0(1/6) + 1(3/12) + 2(7/12) \\
     &= 0 + 3/12 + 14/12 \\
     &= 17/12
\end{align*}
$$

Calculate the second moment $E[X^2]$:
$$
\begin{align*}
E[X^2] &= 0^2(1/6) + 1^2(3/12) + 2^2(7/12) \\
       &= 0 + 3/12 + 28/12 \\
       &= 31/12
\end{align*}
$$

Calculate the variance $Var(X)$:
$$
\begin{align*}
Var(X) &= E[X^2] - (E[X])^2 \\
       &= \frac{31}{12} - \left(\frac{17}{12}\right)^2 \\
       &= \frac{372}{144} - \frac{289}{144} \\
       &= \frac{83}{144} \\
       &\approx 0.5764
\end{align*}
$$
