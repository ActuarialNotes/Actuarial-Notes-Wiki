---
id: "p-199"
exam: "Probability"
topic: "Bayes Theorem"
learning_objective: "General Probability"
difficulty: easy
type: multiple-choice
wiki_link:
  - Concepts/Bayes+Theorem
  - Concepts/The+Law+of+Total+Probability
  - Concepts/Conditional+Probability
answer: "A"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:de568adf68e601c1821a53cc92fc016fcc254542ea3907d5aac6f5c79f0f3380
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-199.md
---

An insurance company studies back injury claims from a manufacturing company. The insurance company finds that 40% of workers do no lifting on the job, 50% do moderate lifting, and 10% do heavy lifting.

During a given year, the probability of filing a claim is 0.05 for a worker who does no lifting, 0.08 for a worker who does moderate lifting, and 0.20 for a worker who does heavy lifting.

A worker is chosen randomly from among those who have filed a back injury claim.

Calculate the probability that the worker's job involves moderate or heavy lifting.

- A) 0.75
- B) 0.81
- C) 0.85
- D) 0.86
- E) 0.89

## Explanation

By the Law of Total Probability:
$$
\begin{align*}
P(\text{claim}) &= 0.40(0.05) + 0.50(0.08) + 0.10(0.20) \\
                &= 0.02 + 0.04 + 0.02 \\
                &= 0.08
\end{align*}
$$

$$
\begin{align*}
P(\text{moderate or heavy} \mid \text{claim}) &= \frac{0.50(0.08) + 0.10(0.20)}{0.08} \\
                                              &= \frac{0.04 + 0.02}{0.08} \\
                                              &= \frac{0.06}{0.08} \\
                                              &= \mathbf{0.75}
\end{align*}
$$
