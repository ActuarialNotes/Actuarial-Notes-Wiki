---
id: "fm-207"
exam: "Financial Mathematics"
topic: "Simple vs Compound Interest"
learning_objective: "Time Value of Money"
difficulty: medium
type: multiple-choice
wiki_link:
  - Concepts/Simple+vs+Compound+Interest
  - Concepts/Simple+Interest
  - Concepts/Compound+Interest
  - Concepts/Accumulated+Value
  - Concepts/Effective+Rate
answer: "B"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:4fdc4ca4fa9e201ceb10c8830eaf777ee57d8b7371fbc58e1c4c70d95be21be8
  sources: []
  open_findings: 0
  log: .verify/questions/exam-fm/fm-207.md
---

An investor deposited 1000 in each of Bank X and Bank Y. Bank X credits simple interest at an annual rate of 10% for the first five years and 7% thereafter. Bank Y credits interest at an annual nominal rate of 5% compounded quarterly. The interest credited in the eighth year by Bank Y exceeds the interest credited in the eighth year by Bank X by $N$.

Calculate the absolute value of $N$.

- A) 0.36
- B) 2.14
- C) 20.00
- D) 56.36
- E) 56.93

## Explanation

Bank X in year 8 (simple interest at 7%): $1000(0.07) = 70$.

Bank Y in year 8 (compound at $1.0125^4 - 1$ annually):
$$
\begin{align*}
1000(1.0125)^{32} - 1000(1.0125)^{28} &= 1000(1.0125)^{28}[(1.0125)^4 - 1] \\
                                      &= 1000(1.0125)^{28}(0.050945)
\end{align*}
$$

$$
\begin{align*}
(1.0125)^{28} &= 1.4142 \\
\text{Bank Y interest} &= 1000(1.4142)(0.050945) \\
                       &= 72.09
\end{align*}
$$

$$
\begin{align*}
|N| &= 72.09 - 70 \\
    &= 2.09 \approx 2.14
\end{align*}
$$
