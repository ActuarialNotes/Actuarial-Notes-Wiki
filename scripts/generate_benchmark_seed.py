...
# Real OSFI-derived rows, GENERATED by scripts/etl_osfi.py (optional — folded in
# when present so the segmented Personal P&C figures come from the regulatory
# returns rather than hand-curated placeholders).
OSFI_CSV_PATH = REPO / "scripts" / "osfi_benchmark_rows.csv"
...
def _read_csv(path):
    """Read one seed CSV, skipping '#'-prefixed comment/banner lines."""
...
def read_rows():
    ...
    # Fold in the generated OSFI rows when present (real regulatory-return data).
    if OSFI_CSV_PATH.exists():
        rows += _read_csv(OSFI_CSV_PATH)
    if not rows:
...
def build_documents(rows):
    ...
    jurisdiction_provinces accumulates every province its metrics touch (a single
    OSFI return covers many provinces), preserving insertion order.
    """
    docs = {}
    for r in rows:
        url = r["source_url"]
        ...
        prov = r.get("province", "").strip()
        if prov and prov != "ATL" and prov not in docs[url]["provinces"]:
            docs[url]["provinces"].append(prov)
    return docs
...
def render(rows, docs):
    ...
    out.append("--")
    out.append("-- Seeds synthetic source-of-record documents + benchmark metrics so the")
    out.append("-- Benchmarks tab has real numbers to show. Segmented Personal P&C figures")
