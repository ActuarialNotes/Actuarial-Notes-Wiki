---
id: "p-334"
exam: "Probability"
topic: "Conditional Probability"
learning_objective: "General Probability"
difficulty: medium
type: multiple-choice
wiki_link:
  - Concepts/Conditional+Probability
  - Concepts/Bayes+Theorem
answer: "A"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:c13602698f5d2913ef89082522db92217ca071e68d4e225957c9eb8418674180
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-334.md
---

A group of health insurance policyholders is composed of 60% men and 40% women. Of the male policyholders, 20% are smokers. Given that a policyholder from the group smokes, the probability that the policyholder is female is 20%.

Calculate the percentage of female policyholders who are smokers.

- A) 7.50%
- B) 8.00%
- C) 12.00%
- D) 13.33%
- E) 20.00%

## Explanation

$P(\text{smoke}\cap\text{male}) = 0.60 \times 0.20 = 0.12$

Let $P(\text{smoke}\cap\text{female}) = p$. Then $P(\text{smoke}) = 0.12+p$.

$$P(\text{female}\mid\text{smoke}) = \frac{p}{0.12+p} = 0.20$$

$$
\begin{align*}
p &= 0.20(0.12+p) \\
  &= 0.024+0.20p \implies 0.80p = 0.024 \implies p = 0.03
\end{align*}
$$

$$
\begin{align*}
P(\text{smoke}\mid\text{female}) &= \frac{0.03}{0.40} \\
                                 &= \mathbf{7.5\%}
\end{align*}
$$
