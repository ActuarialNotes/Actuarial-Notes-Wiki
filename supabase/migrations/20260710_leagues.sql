...
-- promotion/relegation. Leagues are PER EXAM — a student studying multiple
-- exams competes in a separate league for each. Users who join an exam's league
-- are grouped into cohorts of up to 30 at their tier for the current UTC week
-- and ranked by the XP they earn on that exam; when the week rolls over, the top
-- of each cohort promotes and the bottom (plus anyone who earned 0 XP) relegates.
-- Full design: docs/leagues.md.
--
...
--
-- The `exam` column is the exam_progress key ('P', 'FM', 'MAS-I', …), matching
-- EXAM_LABEL_TO_ID / wikiExamIdToProgressKey on the client.

-- ── Tables ────────────────────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS league_cohorts (
  id         uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  exam       text        NOT NULL,                               -- exam_progress key
...
CREATE INDEX IF NOT EXISTS league_cohorts_exam_week_tier_idx
  ON league_cohorts (exam, week_start, tier);

CREATE TABLE IF NOT EXISTS league_members (
  cohort_id    uuid        NOT NULL REFERENCES league_cohorts(id) ON DELETE CASCADE,
  user_id      uuid        NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  exam         text        NOT NULL,                      -- denormalized from the cohort
  ...
  -- One membership per user per exam at a time (rollover deletes old cohorts,
  -- so this never collides across weeks).
  UNIQUE (user_id, exam)
);

-- Per-user, per-exam league state that must survive the weekly membership wipe:
-- the tier ladder position, the opt-in choice, the identity snapshot used to
-- re-enroll each week, and last week's result for the "you were promoted!" banner.
...
-- Place a user into an open (< 30 member) cohort for the given exam/tier/week,
-- creating one if none has room. The cap is soft: a concurrent join can overfill
-- a cohort by one, which is harmless. ON CONFLICT DO NOTHING makes re-enrolment
-- idempotent (user_id+exam is UNIQUE).
...
BEGIN
  ...
  WHERE c.week_start = p_week AND c.tier = p_tier AND c.exam = p_exam
  ...
  IF v_cohort IS NULL THEN
    INSERT INTO league_cohorts (exam, tier, week_start)
    VALUES (p_exam, p_tier, p_week)
    RETURNING id INTO v_cohort;
  END IF;

  INSERT INTO league_members (cohort_id, user_id, exam, display_name, avatar_url)
  VALUES (
    v_cohort,
    p_user,
    p_exam,
...
REVOKE ALL ON FUNCTION league_assign_member(uuid, text, smallint, date, text, text) FROM public;
...
-- re-enroll everyone still opted in at their new tier. Runs across all exams.
--
...
BEGIN
  ...
  SET
    ...
    updated_at       = now()
  FROM results r
  WHERE u.user_id = r.user_id AND u.exam = r.exam;
  ...
  PERFORM league_assign_member(u.user_id, u.exam, u.tier, v_week, u.display_name, u.avatar_url)
  FROM user_leagues u
  WHERE u.opted_in;
END;
$$;
...
-- Opt in to an exam's league: snapshot the caller's display name + avatar and
-- enroll them in a current-week cohort at their tier for that exam. Returning
-- users resume their stored tier; first-timers start at Bronze. Returns the
-- tier joined.
CREATE OR REPLACE FUNCTION join_league(p_exam text, p_display_name text, p_avatar_url text)
RETURNS smallint
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  END IF;
  IF p_exam IS NULL OR trim(p_exam) = '' THEN
    RAISE EXCEPTION 'p_exam is required';
  END IF;

  PERFORM league_rollover_if_due();

  INSERT INTO user_leagues (user_id, exam, opted_in, display_name, avatar_url)
  VALUES (v_user_id, p_exam, true, p_display_name, p_avatar_url)
  ON CONFLICT (user_id, exam) DO UPDATE
    SET opted_in     = true,
  ...
  PERFORM league_assign_member(
    v_user_id, p_exam, v_tier, league_week_start(), p_display_name, p_avatar_url
  );
...
REVOKE ALL ON FUNCTION join_league(text, text, text) FROM public;
GRANT EXECUTE ON FUNCTION join_league(text, text, text) TO authenticated;

-- Opt out of an exam's league: delete this week's membership (and with it the
-- shared name/avatar/XP) and erase the snapshots from user_leagues. Tier and
-- last result are retained so a returning user resumes where they left off.
CREATE OR REPLACE FUNCTION leave_league(p_exam text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  DELETE FROM league_members WHERE user_id = v_user_id AND exam = p_exam;
  ...
  SET opted_in     = false,
      ...
      avatar_url   = NULL,
      updated_at   = now()
  WHERE user_id = v_user_id AND exam = p_exam;
END;
$$;

REVOKE ALL ON FUNCTION leave_league(text) FROM public;
GRANT EXECUTE ON FUNCTION leave_league(text) TO authenticated;

-- Add quiz XP to the caller's weekly total for an exam. Returns the new
-- weekly_xp, or NULL if the caller isn't a member of that exam's league — the
-- client calls this after every quiz, so the non-member case is a silent no-op.
CREATE OR REPLACE FUNCTION record_league_xp(p_exam text, p_amount integer)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  RETURN v_xp;  -- NULL when not a member of this exam's league
END;
$$;

REVOKE ALL ON FUNCTION record_league_xp(text, integer) FROM public;
GRANT EXECUTE ON FUNCTION record_league_xp(text, integer) TO authenticated;

-- Current standings of the caller's cohort for an exam, ranked by weekly XP
-- (earlier join breaks ties). Empty set when the caller isn't a member. This is
-- the ONLY way clients read league_members, and it deliberately exposes no user
-- ids: the caller finds themself via is_self.
CREATE OR REPLACE FUNCTION get_league_board(p_exam text)
RETURNS TABLE (
  rank         integer,
  display_name text,
  avatar_url   text,
...
BEGIN
  ...
  FROM league_members m
  WHERE m.user_id = v_user_id AND m.exam = p_exam;
...
REVOKE ALL ON FUNCTION get_league_board(text) FROM public;
GRANT EXECUTE ON FUNCTION get_league_board(text) TO authenticated;

-- Ensure realtime delivers changes for the user's own league state rows (the
