...
- Outcomes in $S$ must be mutually exclusive (no two can occur simultaneously) and
- Outcomes in $S$ must be collectively exhaustive (together they cover every possibility).
...
> An experiment records whether a policyholder files a claim and, if so, classifies the loss as small ($\leq\$1{,}000$) or large ($>\$1{,}000$). Write the sample space.
>
> > [!answer]- Answer
