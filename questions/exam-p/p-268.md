---
id: "p-268"
exam: "Probability"
topic: "Binomial Distribution"
learning_objective: "Univariate Random Variables"
difficulty: medium
type: multiple-choice
wiki_link:
  - Concepts/Binomial+Distribution
  - Concepts/Expected+Value
answer: "D"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:572898ea30a2a4b525ed9a1f5ead8ce00338458ac5e40231d4ce3900e8b92924
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-268.md
---

A farmer purchases a five-year insurance policy that covers crop destruction due to hail. Over the five-year period, the farmer will receive a benefit of 20 for each year in which hail destroys his crop, subject to a maximum of three benefit payments. The probability that hail will destroy the farmer's crop in any given year is 0.5, independent of any other year.

Calculate the expected benefit that the farmer will receive over the five-year period.

- A) 30
- B) 34
- C) 40
- D) 46
- E) 50

## Explanation

$N \sim \text{Binomial}(5, 0.5)$. Payment $= 20\min(N, 3)$.

$$
\begin{align*}
E[\min(N,3)] &= \sum_{k=0}^{2} P(N > k) \\
             &= P(N\geq1)+P(N\geq2)+P(N\geq3)
\end{align*}
$$

$$P(N=0) = 1/32, \quad P(N \geq 1) = 31/32$$

$$
\begin{align*}
P(N \leq 1) &= (1+5)/32 \\
            &= 6/32, \quad P(N \geq 2) = 26/32
\end{align*}
$$

$$
\begin{align*}
P(N \leq 2) &= (1+5+10)/32 \\
            &= 16/32, \quad P(N \geq 3) = 16/32
\end{align*}
$$

$$E[\min(N,3)] = \frac{31+26+16}{32} = \frac{73}{32}$$

$$
\begin{align*}
E[\text{payment}] &= 20 \times \frac{73}{32} \\
                  &= \frac{1460}{32} \\
                  &\approx \mathbf{45.6 \approx 46}
\end{align*}
$$
