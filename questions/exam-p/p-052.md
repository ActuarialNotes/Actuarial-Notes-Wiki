---
id: "p-052"
exam: "Probability"
topic: "Expected Value"
learning_objective: "Univariate Random Variables"
difficulty: medium
type: multiple-choice
wiki_link:
  - Concepts/Expected+Value
  - Concepts/Continuous+Univariate+Distributions
answer: "C"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:57ccf73cdc67c4ae2e4d4cf8281ac511c7abe02e9d3ed44b148b0a12bf1340d1
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/questions/exam-p/p-052.md
---

An insurance company's monthly claims are modeled by a continuous, positive random variable $X$, whose probability density function is proportional to $(1+x)^{-4}$, for $0 < x < \infty$.

Calculate the company's expected monthly claims.

- A) 1/6
- B) 1/3
- C) 1/2
- D) 1
- E) 3

## Explanation

First, find the constant $k$ such that $f(x) = k(1+x)^{-4}$ integrates to 1 over $[0, \infty)$.
$$
\begin{align*}
1 = \int_{0}^{\infty} k(1+x)^{-4} dx &= \left[ -\frac{k}{3}(1+x)^{-3} \right]_0^{\infty} \\
                                     &= \frac{k}{3} \implies k = 3
\end{align*}
$$
So $f(x) = 3(1+x)^{-4}$.

The expected value is:
$$E[X] = \int_{0}^{\infty} x \cdot 3(1+x)^{-4} dx$$
Let $u = 1+x$, then $x = u-1$ and $dx = du$. The bounds change from $0 \to \infty$ to $1 \to \infty$.
$$
\begin{align*}
E[X] &= \int_{1}^{\infty} 3(u-1)u^{-4} du \\
     &= \int_{1}^{\infty} (3u^{-3} - 3u^{-4}) du \\
     &= \left[ -\frac{3}{2}u^{-2} + u^{-3} \right]_1^{\infty} \\
     &= 0 - \left( -\frac{3}{2} + 1 \right) \\
     &= \frac{1}{2}
\end{align*}
$$
