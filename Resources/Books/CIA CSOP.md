---
Title: "Standards of Practice"
Author: "Canadian Institute of Actuaries"
Year: "2026"
date: "2026"
Publisher: "Actuarial Standards Board (Canada)"
Type: "Standards of Practice"
Available from: "[cia-ica.ca](https://www.cia-ica.ca/app/themes/wicket/custom/dl_file.php?p=713004&fid=713005)"
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:c84867f5cfe70b9c9420997fab5cef3fd2da21bdfbd317da2d2f0d08a85053a8
  sources: []
  open_findings: 0
  open_critical: 0
  log: .verify/Resources/Books/CIA CSOP.md
---
![[CIA CSOP - Cover.svg]]

The **Consolidated [[Standards of Practice]]** — the binding standards for actuarial work in Canada, promulgated by the Actuarial Standards Board and adopted by the CIA. Read for [[Exam 6C (CAS)|Exam 6C]] objective A1 and objectives C1–C5. The syllabus assigns the April 1, 2026 version, sections **1240, 1400, 1510, 1520, 1600, 1700, 2100, 2200, 2300, 2400, 2500, 2600, 8100 and 8200**.

## What "standards of practice" means here

**[[Accepted Actuarial Practice]]** is defined in the standards themselves as *the manner of performing work in accordance with these Standards of Practice*. That definition is load-bearing: the [[Insurance Companies Act]] requires the [[Appointed Actuary|appointed actuary]] to value [[Insurance Contract Liabilities|policy liabilities]] in accordance with accepted actuarial practice, so the standards are given legal force by statute rather than only professional force. Text set **in a box** is the standard; text outside a box is **explanatory text** and is not binding.

## The assigned sections

### General practice (1000)

- **1240** — [[Materiality]] — the actuary selects a materiality standard for the work, and it depends on the *user* and the use
- **1400** — The actuary's use of another's work, data and assumptions (see [[CIA Reliance]])
- **1510** — Selection of assumptions and methods, and the margin for adverse deviations where one applies
- **1520** — [[Subsequent Events]] — events after the calculation date, and the decision tree for whether they are taken into account, disclosed, or neither
- **1600** — Reporting — the report, its users, and what must be disclosed
- **1700** — Control of the work, including [[Peer Review|review]] and documentation

### Insurance practice (2000)

- **2100** — Scope of the insurance standards
- **2200** — The [[Appointed Actuary]] — the role, its reporting lines, and the [[Duty to Report|duty to report]] to the board and the regulator
- **2300** — Valuation of [[Insurance Contract Liabilities|insurance contract liabilities]]
- **2400** — The [[Appointed Actuary's Report|actuary's report]] on insurance contract liabilities, and the [[Statement of Actuarial Opinion|opinion]]
- **2500** — **[[FCT|Financial condition testing]]** — the base scenario, adverse scenarios, [[Ripple Effect|ripple effects]], corrective management actions, and the opinion on satisfactory financial condition
- **2600** — **Ratemaking** — the section assigned for objective A1: rates should not be excessive, inadequate or [[Unfair Discrimination|unfairly discriminatory]], and the actuary's obligations in selecting data, methods and assumptions for a rate

### Public personal injury compensation plans (8000)

- **8100** — Scope — the plans this part applies to ([[Workers Compensation Insurance|workers' compensation]] boards, public auto plans)
- **8200** — Valuation and funding of a public personal injury compensation plan

## How the exam uses it

Almost every Domain C question that asks what the actuary *must* do resolves to one of these sections, and the CIA educational notes on the syllabus are all written as guidance *under* them — an educational note describes useful practice, the standard compels it. Section 2500 (FCT) and section 2400 (the report and opinion) are the two most heavily examined.

## Related readings
- [[CIA Valuation]] — the annual guidance to the appointed actuary, written against these standards
- [[CIA IFRS 17 - Comparison]] — what IFRS 17 changed relative to the pre-2023 standards
- [[OSFI Memorandum]] — what OSFI additionally requires in the Appointed Actuary's Report

## Links
- [Standards of Practice (CIA)](https://www.cia-ica.ca/app/themes/wicket/custom/dl_file.php?p=713004&fid=713005)
