---
id: "p-006"
exam: "Probability"
topic: "Conditional Probability"
learning_objective: "General Probability"
difficulty: easy
type: multiple-choice
wiki_link:
  - Concepts/Conditional+Probability
  - Concepts/Bayes+Theorem
answer: "B"
points: 1
verification:
  status: unverified
  confidence: null
  last_checked: null
  last_checked_by: null
  content_hash: sha256:eea24c5c503e0be8dd513658b890b8094395b2d1f0da45c4a00d43bfb9775b17
  sources: []
  open_findings: 0
  log: .verify/questions/exam-p/p-006.md
---

A public health researcher examines the medical records of a group of 937 men who died in 1999 and discovers that 210 of the men died from causes related to heart disease. Moreover, 312 of the 937 men had at least one parent who suffered from heart disease, and, of these 312 men, 102 died from causes related to heart disease.

Calculate the probability that a man randomly selected from this group died of causes related to heart disease, given that neither of his parents suffered from heart disease.

- A) 0.115
- B) 0.173
- C) 0.224
- D) 0.327
- E) 0.514

## Explanation

Let $H$ = event that a death is due to heart disease, $F$ = event that at least one parent suffered from heart disease.

$$
\begin{align*}
P\left[H \cap F^c\right] &= \frac{210 - 102}{937} \\
                         &= \frac{108}{937}
\end{align*}
$$

$$P\left[F^c\right] = \frac{937 - 312}{937} = \frac{625}{937}$$

$$
\begin{align*}
P\left[H \mid F^c\right] &= \frac{P\left[H \cap F^c\right]}{P\left[F^c\right]} \\
                         &= \frac{108}{937} \bigg/ \frac{625}{937} \\
                         &= \frac{108}{625} \\
                         &= 0.173
\end{align*}
$$
