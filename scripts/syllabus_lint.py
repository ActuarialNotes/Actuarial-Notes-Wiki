...
this lint, nothing checked one. It fails a page for:
  ...
  links       every noun phrase of an objective — its numbered line, its
              ...
              what holds every page to it); every objective links at least one concept (its own words or its
...
def _lint_links(rel: str, page: sl.ExamPage, vault: vl.Vault, report: Report, strict, exam: dict) -> None:
    ...
    # Every noun phrase an objective (or a section's preamble, or an objective's
    # sub-items) names should be a note. A chunker can't be sure what a noun
    # phrase is, so this only warns. Headings, bold group labels, `*Key
    # concepts:*` lines and the `**Readings:**` line are not objective text.
    def prose(l: str) -> bool:
        return bool(l.strip()) and not re.match(r"^\s*(\*\*.*\*\*\s*$|#|\*\*Readings)", l) \
            and not sl.KEY_CONCEPTS_RE.match(l)

    for s in page.sections:
        lines = [(ln, l) for ln, l in zip(s.line_nos, s.lines) if l in s.preamble and prose(l)]
        for o in s.objectives:
            lines.append((o.line_no, o.text))
            lines += [(ln, l) for ln, l in zip(o.extra_line_nos, o.extra) if prose(l)]
