...
- `syllabusChapters.ts` — the study guide's **chapters**: an exam page's main learning
  objectives, laid along the bar under the wiki's sticky header
  (`components/wiki/SyllabusChapterBar.tsx`, which measures them via the `data-objective`
  attributes `MarkdownCallout` puts on every weighted `[!example]` callout). Segments are
  sized by **exam weight** rather than page extent — the callouts collapse to identical
  strips, so only the weight says what the syllabus is made of — and scrolling maps into
  the current objective's span so the fill moves as you read. Pure and tested; weights come
  from `lib/examWeight.ts` and are never invented (no weights → even segments). See
  `docs/style-guide.md` §7.5.
